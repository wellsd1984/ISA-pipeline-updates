...
sub make_unique_pair_fasta {
	...
	# $unique{UNIQUE_PAIR_ID}{'total'} = TIMES PAIR IS SEEN
	# and
	# $unique{UNIQUE_PAIR_ID}{$mid} = [TIMES PAIR IS SEEN WITH THIS MID, [@INPUT_LINE_COMPONENT_PARTS], $Linker_seq, $LTR_seq];
	...
	while (my $line = <$data_in>) {
		...
		unless (exists $unique{$tag}{$mid}) {
			...
			$unique{$tag}{$mid} = [1, [@elements], $LTR_seq, $Linker_seq]; # Set the uniqe pair count to 1. Store the relevant sequence data.
			$unique{$tag}{'total'} = 1;
	...
	foreach my $tag (keys %unique) {
		my $tag_count = $unique{$tag}{'total'};
		delete $unique{$tag}{'total'};
		my @sorted_mids = sort { $unique{$tag}{$b}[0] <=> $unique{$tag}{$a}[0] } keys %{$unique{$tag}};
		$unique{$tag}{$sorted_mids[0]}[1][0] .= "#$tag_count"; # Append the unique pair count to the sequence ID w/ MID
		my $newline = join("\t", @{$unique{$tag}{$sorted_mids[0]}[1]}); # Join the line elements with the modified sequence ID to make the output line
		print $out_counts "$newline\n"; # Print the new line to an interim report
		print $outL ">$unique{$tag}{$sorted_mids[0]}[1][0]\n$unique{$tag}{$sorted_mids[0]}[2]\n"; # Print the read 1 sequences to a fasta file
		print $outR ">$unique{$tag}{$sorted_mids[0]}[1][0]\n$unique{$tag}{$sorted_mids[0]}[3]\n"; # Print the read 2 sequences to a fasta file
	}
