...
sub annotate_blat_file {
	my ($LTR,$name,$path2genome,$folder) = @_;
	...
	my @primer_end_bases = split_primer($primer_end); # Returns an array with one base per array element. IUPAC codes are replaces with regex character classes
	
	# Open file to read
	my $blat_file = "$folder/merged_integration_sites.txt";
	...
	while (<$blat_in>) {
		...
		unless (exists $indicator_results{$coordinate}) {
			...
			my ($indicator, $sequence,$pseudo_3LTR) = getseq_fast($elements[1],$elements[3], $elements[4], $elements[5],$path2genome,$LTR, $LTR_junction, \@primer_end_bases);
			...
			$indicator_results{$coordinate} = "$pseudo_3LTR\t$indicator\t$sequence";
		}
	...
	my $out_file = "$folder/final_annotated_integration_sites_report.txt";
	my $out_fuzz = "$folder/annotated_integration_sites_report_with_fuzz.txt";
	open my $blat_out,">", "$out_file" or die "$out_file: $!";
	open my $fuzz_out,">", "$out_fuzz" or die "$out_fuzz: $!";
	print $blat_out "Count\tSeq ID\tMID\tJunction chr\tJunction strand\tJunction base\tBreak point chr\tBreak point strand\tBreak point base\tLength\tPseudo 3LTR\tHIV LTR\tProvirus orientation to chromosome\tPrediction\tUpstream 30\tDownstream 30\tRead 1\tRead 2\n";
	print $fuzz_out "Count\tSeq ID\tMID\tJunction chr\tJunction strand\tJunction base\tBreak point chr\tBreak point strand\tBreak point base\tLength\tPseudo 3LTR\tHIV LTR\tProvirus orientation to chromosome\tPrediction\tUpstream 30\tDownstream 30\tRead 1\tRead 2\n";

	foreach (@sorted_lines) {
		if (defined $data_lines{$_}[13]) {
 			$data_lines{$_}[13] =~ s/no_indicator//;
 			$data_lines{$_}[13] = $1 if $data_lines{$_}[1] =~ s/\#(fuzz\?{0,1})//; 
 		}
...
sub getseq_fast {
	...
	my $primer_bases = pop;
	my ($sequence_ID, $chr, $strand, $IS_base, $path2genome, $LTR) = @_;
	
	# Get the virus orientation relative to the chromosome and the pseudo 3LTR
	my ($virus_strand, $pseudo_3LTR);

	$pseudo_3LTR = "$chr$strand$IS_base" if $LTR eq "3LTR";
	...
	if ($LTR eq "3LTR" and $strand eq "+") {
		...
		$virus_strand = "$LTR\t-";
		my $pseudo_base = $IS_base + 5;
		$pseudo_3LTR = "$chr" . "-" . "$pseudo_base";
	}elsif($LTR eq "5LTR" and $strand eq "-") {
		$virus_strand = "$LTR\t+";
		my $pseudo_base = $IS_base - 5;
		$pseudo_3LTR = "$chr" . "+" . "$pseudo_base";
	...
	if ($chr =~ /HIV-1|HIV1/) {
		my $indicator = my $seq = $pseudo_3LTR = "";
		return ($indicator, $seq, $pseudo_3LTR);
	}
	
	# $file is the path to the chromosome fasta file.
	my $file = $path2genome."/$chr.fa";
...
sub sort_fasta_by_length {
	...
	my $in_file = "$folder/read1_fasta_for_alignment.fa";
	my $out_file = "$folder/read1_sorted_by_length_for_Uclust.fa";
	open my $data_in, '<', $in_file or die "$in_file: $!\n";
	open my $data_out, '>', $out_file or die "$out_file: $!\n";
...
sub resort_Uclust {
	...
	my $uc_file = "$folder/uclust_results.uc";
	...
	open my $in_txt, "<","$folder/read1_sorted_by_length_for_Uclust.fa" or die "$folder/read1_sorted_by_length_for_Uclust.fa: $!\n";
	...
	my $blat_file = "$folder//all_read1_coordinates_for_Uclust.txt";
	$blat_file =~ s/\.gz^//;
	open my $blat_in, "<", $blat_file or die "$blat_file: $!\n";
